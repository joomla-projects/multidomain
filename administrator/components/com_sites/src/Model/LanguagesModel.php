...
class LanguagesModel extends ListModel
{
		...
		protected function getListQuery()
		{
				...
				$query
					...
					->from($db->quoteName('#__sites_languages', 'a'))
					->join(
						'LEFT',
				...
				if (!empty($search)) {
						if (stripos($search, 'id:') === 0) {
								...
								$query->where($db->quoteName('a.idLanguage') . ' = :search')
				...
				$orderCol  = $this->state->get('list.ordering', 'a.idLanguage');
				$orderDirn = $this->state->get('list.direction', 'ASC');

				$query->order($db->escape($orderCol) . ' ' . $db->escape($orderDirn));
				$query->order($db->escape('a.idLanguage') . ' ' . $db->escape($orderDirn));
